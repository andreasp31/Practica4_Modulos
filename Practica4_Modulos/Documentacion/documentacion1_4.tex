...
\begin{enumerate}[start=2]
    ...
    \item Errores encontrados 
    \end{itemize}
    \item Actividad 04 -  Módulo Colegio ............................................................ página 28
    \begin{itemize}
...
El objetivo de la práctica es crear y modificar módulos para seguir adquiriendo conceptos y ser capaces de crear módulos por nuestra cuenta. Nos vamos a basar en ejemplos del 01 al 06 del repositorio que nos proporcionan en el enuncionado: \href{https://github.com/sergarb1/OdooModulosEjemplos}{repositorio de ejemplo}. Primero vamos a tener en cuenta que el primero ejercicio lo vamos a realizar en base al módulo de la práctica anterior de la "lista de tareas" por lo que vamos a seguir usando la práctica anterior.
...
\begin{itemize}
    \item Modelo Ciclo Formativo que puede estar asociado a uno o más de un módulo.
    \item Modelo Módulo que pertenece a un ciclo formativo, tiene que tener alumnos matriculados y tener un profesor que imparta el módulo.
    \item Modelo Alumno con sus atributos y se relaciona con los módulos a los que se matricule.
    \item Modelo Profesor con sus atributos y se relaciona con los módulos a los que se matricule.
\end{itemize}
...
Después en el modelo "Alumno" vamos a necesitar tener por un lado el atributo "id modulos" con una relación de (Many2many) donde el alumno puedee estar en muchos módulos y un módulo tiene varios alumnos(N:M).

Y en el modelo "Profesor" necesitamso el atributo "id modulos" con una relación de (One2many) donde el profesor imparte uno o muchos módulos(1:N).
...
Vamos a empezar creando el archivo módulo "Ciclo" con la siguiente estructura y los atributos que mencionamos ateriormente más los que creamos necesarios para el ciclo.
...
class CicloFormativo(models.Model):
    ...
    codigo_ciclo = fields.Char(string='Codigo',required=True)
    descripcion = fields.Html(string='Descripcion')
    id_modulos = fields.One2many('colegio.modulo','id_ciclo',string='Modulos')
\end{lstlisting}
...
<odoo>
    <!-- Accion para Ciclos -->
    <record id='gestion_colegio.colegio_ciclo_action' model='ir.actions.act_window'>
    ...
    <record id="gestion_colegio.colegio_ciclo_view_list" model="ir.ui.view">
        <field name="name">Lista de Ciclos Formativos</field>
        <field name="model">colegio.ciclo</field>
        <field name="arch" type="xml">
            <list>
                <field name="nombre"/>
                <field name="codigo_ciclo"/>
                <field name="descripcion"/>
            </list>
        </field>
    </record>

    <!-- Vista formulario de Ciclos -->
    <record id="gestion_colegio.colegio_ciclo_view_form" model="ir.ui.view">
        <field name="name">Formulario de Ciclo Formativo</field>
        <field name="model">colegio.ciclo</field>
        <field name="arch" type="xml">
            <form>
                <sheet>
                    ...
                    <notebook>
                        <page string="Modulos">
                            <field name="id_modulos">
                                <list>
                                    <field name="nombre"/>
                                    <field name="codigo_modulo"/>
                                    <field name="horas"/>
                                    <field name="creditos"/>
                                    <field name="curso"/>
                                    <field name="id_profesor" widget="many2one"/>
                                </list>
                            </field>
                        </page>
                    </notebook>
    ...
    <!-- Vista busqueda de Ciclos -->
    <record id="gestion_colegio.colegio_ciclo_view_search" model="ir.ui.view">
...
Luego de tener la vista la añadimos al archivo "manifest", importamos el modelo al archivo "init" de la carpeta "models" y añadirmos los permisos que consideramos por ahora, que posteriormente lo vamos a tener que cambiar.
...
# -*- coding: utf-8 -*-
{
    'name': "Gestion del Colegio Princesa de Espana",
    'summary': """Gestion de ciclos formativos, modulos, alumnos y profesores""",
    'description': """Modulo para la gestion completa de un instituto educativo:""",
    'author': "Andrea Sofia Pais Dos Santos",
    'application': True,
    'category': 'Education',
    'version': '0.1',
    'depends': ['base'],
    'data': [
        # primero seguridad, luego vistas, luego menus
        'security/group.xml',
        'security/ir.model.access.csv',
        
        # Vistas en orden alfabetico para claridad
...
from . import colegio_ciclo
from . import colegio_modulo
from . import colegio_alumno
from . import colegio_profesor
\end{lstlisting}
...
\begin{lstlisting}[style=javascript]
id,name,model_id:id,group_id:id,perm_read,perm_write,perm_create,perm_unlink
access_colegio_ciclo_user,access_colegio_ciclo_user,model_colegio_ciclo,gestion_colegio.grupo_administrativos,1,1,1,1
access_colegio_modulo_user,access_colegio_modulo_user,model_colegio_modulo,gestion_colegio.grupo_administrativos,1,1,1,1
access_colegio_alumno_user,access_colegio_alumno_user,model_colegio_alumno,gestion_colegio.grupo_administrativos,1,1,1,1
access_colegio_profesor_user,access_colegio_profesor_user,model_colegio_profesor,gestion_colegio.grupo_administrativos,1,1,1,1
\end{lstlisting}
...
\begin{center}
    \includegraphics[height=4cm]{images/modulo40.png}
\end{center}
...
\vspace{0.5cm}
\begin{center}
    \includegraphics[height=5cm]{images/modulo41.png}
\end{center}
...
\vspace{0.5cm}
\begin{center}
    \includegraphics[height=5cm]{images/modulo45.png}
\end{center}
...
\vspace{0.5cm}
\begin{center}
    \includegraphics[height=6cm]{images/modulo46.png}
\end{center}
...
class ColegioModulo(models.Model):
    ...
    codigo_modulo = fields.Char(string='Codigo',required=True)
...
<odoo>
    <!-- Accion para Modulo -->
    <record id='gestion_colegio.colegio_modulo_action' model='ir.actions.act_window'>
        <field name="name">Modulos</field>
        <field name="res_model">colegio.modulo</field>
        <field name="view_mode">list,form</field>
    </record>

    <!-- Vista de lista de Modulo -->
    <record id="gestion_colegio.colegio_modulo_view_list" model="ir.ui.view">
        <field name="name">Lista de Modulos</field>
        <field name="model">colegio.modulo</field>
        <field name="arch" type="xml">
            <list>
                <field name="nombre"/>
                <field name="codigo_modulo"/>
    ...
    <!-- Vista de formulario de Modulo -->
    <record id="gestion_colegio.colegio_modulo_view_form" model="ir.ui.view">
        <field name="name">Formulario de Modulo</field>
        <field name="model">colegio.modulo</field>
        <field name="arch" type="xml">
            <form>
                <sheet>
                    <group>
                        ...
                        <group>
                            <field name="curso"/>
                            <field name="id_ciclo" widget="many2one"/>
                            <field name="id_profesor" widget="many2one"/>
                        </group>
                    </group>
    ...
    <!-- Vista de busqueda de Modulo -->
    <record id="gestion_colegio.colegio_modulo_view_search" model="ir.ui.view">
...
Luego hay que hacer lo mismo en toda creación de modelos y vistas, añadirlo al "manifest", importar el modelo al "init" de la carpeta models y añadir los permismos que consideremos. (Ya están completados en el anterior modelo)
...
\vspace{0.5cm}
\begin{center}
    \includegraphics[height=5.5cm]{images/modulo43.png}
\end{center}
...
\vspace{0.3cm}
\begin{center}
    \includegraphics[height=2.4cm]{images/modulo47.png}
\end{center}

\subsection{Modelo Alumno}
...
class ColegioAlumno(models.Model):
    ...
    telefono = fields.Char(string='Telefono del familiar')
    fecha_matriculacion = fields.Date('Fecha de matriculacion',default=fields.Date.today)
    # relaciones que se necesitan 
    id_ciclo = fields.Many2one('colegio.ciclo',string='Ciclo matriculado')
    id_modulo = fields.Many2many('colegio.modulo',string='Modulos matriculados')
...
<odoo>
    <!-- Accion para Alumno -->
    <record id='gestion_colegio.colegio_alumno_action' model='ir.actions.act_window'>
        <field name="name">Alumnos del colegio</field>
        <field name="res_model">colegio.alumno</field>
        <field name="view_mode">list,form</field>
    </record>

    <!-- Vista de lista de Alumno -->
    <record id="gestion_colegio.colegio_alumno_view_list" model="ir.ui.view">
    ...
    <record id="gestion_colegio.colegio_alumno_view_form" model="ir.ui.view">
    ...
    <!-- Vista de busqueda de Alumno -->
    <record id="gestion_colegio.colegio_alumno_view_search" model="ir.ui.view">
        <field name="name">Busqueda de Alumno</field>
        <field name="model">colegio.alumno</field>
        <field name="arch" type="xml">
            <search>
                <field name="nombre"/>
...
\vspace{0.3cm}
\begin{center}
    \includegraphics[height=5cm]{images/modulo44.png}
\end{center}

\subsection{Modelo Profesor}
...
class ColegioProfesor(models.Model):
    ...
    fecha_contratacion = fields.Date('Fecha de contratacion',default=fields.Date.today)
    # relaciones que se necesitan 
    id_modulo = fields.One2many('colegio.modulo','id_profesor',string='Modulos que imparte')
...
<?xml version="1.0" encoding="utf-8"?>
<odoo>
    <!-- Accion para Profesor -->
    <record id='gestion_colegio.colegio_profesor_action' model='ir.actions.act_window'>
        <field name="name">Profesor del colegio</field>
        <field name="res_model">colegio.profesor</field>
        <field name="view_mode">list,form</field>
    </record>

    <!-- Vista de lista de profesor -->
    <record id="gestion_colegio.colegio_profesor_view_list" model="ir.ui.view">
    ...
    <!-- Vista de formulario de Profesor -->
    <record id="gestion_colegio.colegio_profesor_view_form" model="ir.ui.view">
    ...
    <!-- Vista de busqueda de profesor -->
    <record id="gestion_colegio.colegio_profesor_view_search" model="ir.ui.view">
        <field name="name">Busqueda de Profesor</field>
        <field name="model">colegio.profesor</field>
        <field name="arch" type="xml">
            <search>
                <field name="nombre"/>
...
\vspace{0.3cm}
\begin{center}
    \includegraphics[height=4.5cm]{images/modulo42.png}
\end{center}

Ya se realizaron las cuatro modelos y sus pistas teniendo en cuenta las relaciones entre las entidades. 

\subsection{Configuración de seguridad}

El objetivo ahora es tras crear el módulo correctamente con sus vistas vamos a implementar dos roles: el de DIRECTOR (va a poder acceder a todo) y PROFESOR (solo tendrá permisos de lectura sobre las vistas). Para ello vamos a tener que modificar nuestros archivos de seguridad. Empezando por el archivo "group.xml".

En el archivo "group.xml" vamos a crear 2 grupos, el grupo director que es el que va a estar por defecto y el grupo profesor.

\vspace{2cm}
\begin{lstlisting}[style=javascript]
<?xml version="1.0" encoding="utf-8"?>
<odoo>
    <!-- Grupo para Directores (acceso completo) -->
    <record id="group_director" model="res.groups">
        <field name="name">Director</field>
        <field name="category_id" ref="base.module_category_services"/>
        <!-- Asigna el usuario admin como director inicial -->
        <field name="users" eval="[(4, ref('base.user_admin'))]" />
    </record>
    
    <!-- Grupo para Profesores (solo lectura de profesores) -->
...
Y ahora en el archivo "ir.model.access.csv" vamos a determinar los permismos que va a tener cada rol en cada vista. Y los usuarios normales no va a poder acceder a nada por lo que el módulo no aparecerá. El director podrá ver todo, modificar, elmiminar y crear mientras que el profesor solo podrá ver la vista profesor y no podrá modificar nada.

\vspace{0.5cm}
\begin{lstlisting}[style=javascript]
id,name,model_id:id,group_id:id,perm_read,perm_write,perm_create,perm_unlink
# permisos del grupo director
access_colegio_ciclo_director,access_colegio_ciclo_director,model_colegio_ciclo,gestion_colegio.group_director,1,1,1,1
access_colegio_modulo_director,access_colegio_modulo_director,model_colegio_modulo,gestion_colegio.group_director,1,1,1,1
access_colegio_alumno_director,access_colegio_alumno_director,model_colegio_alumno,gestion_colegio.group_director,1,1,1,1
access_colegio_profesor_director,access_colegio_profesor_director,model_colegio_profesor,gestion_colegio.group_director,1,1,1,1
# permisos del grupo profesor
access_colegio_profesor_profesor,access_colegio_profesor_profesor,model_colegio_profesor,gestion_colegio.group_profesor,1,0,0,0
\end{lstlisting}

Al realizar los cambios, actualizamos el módulo.

\vspace{0.3cm}
\begin{center}
    \includegraphics[height=4cm]{images/modulo48.png}
\end{center}

Nos dirigimos a ajustes -> administrar usuarios y más abajo vemos una sección que es la tiene los 2 roles que hemos creado, por defecto vemos que está director puesto que nos permite acceder a todo el módulo.

\vspace{0.3cm}
\begin{center}
    \includegraphics[height=8cm]{images/modulo49.png}
\end{center}

Mientas que si cambiamos al rol de profesor, solo nos deja ver la vista de profesores.

\vspace{0.3cm}
\begin{center}
    \includegraphics[height=6cm]{images/modulo50.png}
\end{center}
